...
bool ask_makeCorrectMove(int& x, int& y, Board* board) {
    ...
    do 
    {
        std::cout << "Twoj ruch \"0\", podaj wspolzedna  x: ";
        std::cin >> x;
        std::cout << "Twoj ruch \"0\", podaj wspolzedna  y: ";
        std::cin >> y;
        std::cout << "\n";
...
int ask_to_ile_zeby_wygrac(int size)
{
    int x;
    do
    {
        std::cout << "\nIlosc znaczkow w kolejnosci zeby wygrac: ";
        std::cin >> x;
...
int main()
{
    int x, y;
	int size = setSize();
    int ile_zeby_wygrac = ask_to_ile_zeby_wygrac(size);

    Board* board = new Board(size);
    Player player_i('O', board, ile_zeby_wygrac);
    Player player_ai('X', board, ile_zeby_wygrac);
