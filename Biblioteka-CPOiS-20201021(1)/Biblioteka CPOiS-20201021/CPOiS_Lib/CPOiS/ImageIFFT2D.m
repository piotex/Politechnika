%% ImageIFFT2D
% 
%  Returns the two-dimensional discrete inverse Fourier transform of an image using a fast Fourier transform algorithm
% 
function ImageIFFT2D(block)
...
block.InputPort(1).DatatypeID  = 0; %doubles
...
% block.OutputPort(1).Data = uint8(real(ifft2(sigVal)));
block.OutputPort(1).Data = (real(ifft2(ifftshift(sigVal))));
%endfunction
