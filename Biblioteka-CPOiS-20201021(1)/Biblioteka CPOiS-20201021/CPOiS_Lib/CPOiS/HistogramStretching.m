...
% 
function HistogramStretching(block)
...
if min(min(double(sigVal))) == max(max(sigVal))
    block.OutputPort(1).Data = uint8(zeros(size(sigVal)));
else
    block.OutputPort(1).Data = imadjust(sigVal,[min(min(double(sigVal)))/255,max(max(double(sigVal)))/255],[0,1]);
end
%endfunction
